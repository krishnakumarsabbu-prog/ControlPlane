...
import type { SystemStats, Project } from '../types'
...
export default function TopBar({ stats, projects }: TopBarProps) {
  const runningCount = projects.filter(p => p.status === 'running').length

  const cpuColor =
    stats.cpu > 80 ? 'text-error' : stats.cpu > 60 ? 'text-starting' : 'text-running'

  const ramPct = Math.round((stats.ram / stats.ramTotal) * 100)
  const ramColor =
    ramPct > 80 ? 'text-error' : ramPct > 60 ? 'text-starting' : 'text-running'
